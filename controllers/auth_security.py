...
def auth_login_post():
    ...
    if user:
        ...
        if not mdp_ok:
            flash(u'Mot de passe incorrect.', 'alert-warning')
        ...
        flash(u'Ce login n\'existe pas.', 'alert-warning')
        return redirect('/login')
...
def auth_signup_post():
    ...
    user = mycursor.fetchone()
    if user:
        flash(u'Cette adresse mail ou ce login est déjà utilisé.', 'alert-warning')
